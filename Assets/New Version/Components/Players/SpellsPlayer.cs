...
public class SpellsPlayer : MonoBehaviour
{
	...
	public void SpawnObject(int spawnablePrefabId, float px, float py, float pz, float rx, float ry, float rz, float rw)
	{
		GameObject obj = Instantiate(spawnableObjects[spawnablePrefabId], new Vector3(px, py, pz), new Quaternion(rx, ry, rz, rw));
	}

	public void SpawnChildObject(int spawnablePrefabId, float px, float py, float pz, float rx, float ry, float rz, float rw)
	{
